...
export default function AboutPillarCard({ item, className = '' }: AboutPillarCardProps) {
  ...
  return (
    <div
      className={`rounded-3xl bg-white border border-slate-200/80 hover:border-gold-border card-hover-gold-accent shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-500 overflow-hidden flex flex-col justify-between ${className}`}
    >
      {/* Top Cover Image */}
      ...
      <div className="p-6 sm:p-7 flex flex-col justify-between flex-1 space-y-4">
        ...
        <Link
          href="/about"
          className="pt-4 border-t border-slate-100 flex items-center justify-between text-xs font-medium text-slate-500 hover:text-gold-dark transition-colors group"
        >
          <span>Explore Hub Responsibilities</span>
