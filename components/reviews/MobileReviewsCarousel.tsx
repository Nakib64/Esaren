          ...
          {TESTIMONIALS.map((t) => (
            ...
            >
              <div className="w-full max-w-sm mt-20 flex flex-col gap-3">
                {/* 1. White Quote Card */}
                <div className="rounded-3xl bg-white border border-slate-200/80 p-6 shadow-2xl relative overflow-hidden flex flex-col justify-between">
                  <Quote className="absolute top-5 right-5 w-16 h-16 text-slate-100 pointer-events-none" />
                  <div className="space-y-3 relative z-10">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-[10px] font-mono tracking-widest text-[#1e3a8a] uppercase bg-[#f8fafc] px-3 py-1 rounded-full border border-slate-200 font-semibold">
                        {t.organization}
                      </span>
                      <span className="text-[10px] font-mono font-bold text-[#0c1a30] bg-[#f4f1ea] px-3 py-1 rounded-full">
                        Impact: {t.metric}
                      </span>
                    </div>
                    <h3 className="text-lg font-serif font-bold text-[#0c1a30] leading-snug">
                      "{t.title}"
                    </h3>
                  ...
                  <div className="flex gap-1 text-[#1e3a8a] pt-3 mt-3 border-t border-slate-100 relative z-10">
                    {[...Array(5)].map((_, i) => (
                ...
                <div className="rounded-3xl bg-[#0c1a30] text-white p-5 shadow-xl space-y-2">
                  <img
                    src={t.photo}
                    alt={t.author}
                    className="w-12 h-12 rounded-xl object-cover border-2 border-white/20 shadow"
                  />
                  <div>
                    <h4 className="text-base font-bold font-serif">{t.author}</h4>
                    <p className="text-xs font-mono text-slate-300">{t.role}</p>
                    <p className="text-xs font-mono text-[#60a5fa]">{t.location}</p>
                  </div>
                </div>

                {/* 3. White Endorsement Counter Card */}
                <div className="rounded-2xl bg-white border border-slate-200 px-5 py-3 flex items-center justify-between shadow-sm">
                  ...
                  <span className="text-lg font-mono font-bold text-[#0c1a30]">
                    {t.id}
