...
import { CheckCircle2, Layers, Cpu, Wrench, ShieldCheck } from 'lucide-react';
  ...
  return (
    <div>
      <SubpageHero
        ...
        graphicSrc="/images/hero_services_transparent.webp"
        primaryCtaText="Explore Capabilities"
        primaryCtaLink="#specialties"
        secondaryCtaText="Request EPC Consultation"
        secondaryCtaLink="/contact"
        pillars={[
          { icon: Layers, title: 'EPC Turnkey', subtitle: 'Engineering Lifecycle' },
          { icon: Wrench, title: 'Roads & Telecom', subtitle: 'Civil Infrastructure' },
          { icon: ShieldCheck, title: 'PMO Control', subtitle: 'Budget & Schedule' },
        ]}
      />

      {/* Page 11: Key Competencies for Project Management */}
      <CompetenciesSection />
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 space-y-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {EPC_PHASES.map((phase, idx) => (
              <div
                key={phase.phase}
                className="rounded-3xl bg-white border border-slate-200 hover:border-gold-border card-hover-gold-accent p-8 shadow-sm hover:shadow-xl transition-all duration-400 space-y-6"
              >
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 uppercase tracking-wider mt-1">{phase.tagline}</p>
                </div>
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 space-y-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {INFRASTRUCTURE_SPECIALTIES.map((spec) => (
              <div
                key={spec.title}
                className="rounded-2xl bg-white border border-slate-200/90 hover:border-gold-border card-hover-gold-accent p-6 shadow-sm hover:shadow-md transition-all space-y-3"
              >
